...
let [@inline never] probably_raise v =
  ...
  | w -> w + 1
  | exception ExnA -> (print_endline "Caught an ExnA!";
...
let main v =
  (try
    ...
    | ExnB -> print_endline "Caught an ExnB!"; Printexc.print_backtrace stdout)
  with
  | ExnC -> print_endline "Caught an ExnC!"; Printexc.print_backtrace stdout);
  v + 1
